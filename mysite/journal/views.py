...
class ExerciseListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Group exercises by name using itertools.groupby
        grouped_exercises = []
        exercises = sorted(exercises, key=lambda exercise: exercise.exercise_name)  # Sort exercises by name
        for name, group in itertools.groupby(exercises, key=lambda exercise: exercise.exercise_name):
...
class AddExerciseCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data.get('custom_exercise_name'):
            custom_exercise_name = form.cleaned_data['custom_exercise_name']
            form.instance.custom_exercises.add(self.request.user)
            form.instance.exercise_name = custom_exercise_name
        else:
            form.instance.exercise_name = form.cleaned_data['exercise_name']
...
def exercise_workouts(request, exercise_name):
    ...
    workouts = Workout.objects.filter(exercises__exercise_name__iexact=exercise_name).distinct()
    ...
    for workout in workouts:
        ...
        exercises = workout.exercises.filter(exercise_name__iexact=exercise_name)
...
def personal_records_by_weight(request):
    ...
    if query:
        exercises = exercises.filter(
            Q(exercise_name__icontains=query)
        )

    top_weight_workouts = {}

    for exercise in exercises:
        workouts = Workout.objects.filter(exercises__exercise_name=exercise.exercise_name).annotate(
            max_weight=Max('exercises__weight')).order_by('-max_weight')[:3]

        top_weight_workouts[exercise.exercise_name] = []

        for workout in workouts:
            exercise_instance = workout.exercises.filter(exercise_name=exercise.exercise_name).order_by('-weight').first()

            if exercise_instance:
                top_weight_workouts[exercise.exercise_name].append((workout, exercise_instance))

    context = {
        'top_weight_workouts': top_weight_workouts,
        'query': query if query else '',
        'all_exercises': Exercise.objects.values_list('exercise_name', flat=True).distinct()
    }
...
def personal_records_by_reps(request):
    ...
    for exercise in exercises:
        ...
        top_rep_workouts[exercise.exercise_name] = []

        for workout in workouts:
            exercise_instance = workout.exercises.filter(exercise_name=exercise.exercise_name).order_by('-rep').first()

            if exercise_instance:
                top_rep_workouts[exercise.exercise_name].append((workout, exercise_instance))
